...
def export_reports():
    ...
    try:
        ...
        # Using the older method for a full report
        query = '''
            SELECT s.name, s.enrollment_no, a.session_date as date, a.marked_at as time
            FROM attendance a
            JOIN students s ON a.student_id = s.id
            ORDER BY a.session_date DESC, a.marked_at DESC
        '''
        rows = db_manager._execute(query, fetchall=True)
